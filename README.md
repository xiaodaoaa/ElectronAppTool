...
└── 仅通过 window.electronAPI / window.api 与主进程通信，绝不直接 import Node/Electron 模块
```
...
- 跨进程共享类型放 `src/shared/`，三端通过 `@shared/*` 别名引用

### IPC 模式

所有项目使用同一套 IPC 规范，通过 `window.electronAPI`（前四个项目）或 `window.api`（ERabbitMQToolPlus）通信：
...
ERabbitMQToolPlus 使用 Electron 43，**不支持 Win7**。
