...
Electron 桌面工具集合，四个独立子项目组成的 monorepo。
...
- **EWebsocketTool** 和 **EWebsocketMan**（Win7 兼容）：pack 命令包含 `-c.electronDist="node_modules/electron/dist" -c.electronVersion="22.3.27"`，强制 electron-builder 使用本地安装的 Electron 22 二进制。
...
**方式 A — 独立 electron/ 目录**（EHttpServerTool、EWebsocketTool、EPinyinStudy）：
...
EWebsocketTool 和 EWebsocketMan 使用 **Electron 22.3.27**（最后一个支持 Windows 7 的版本，Electron 23+ 已放弃 Win7 支持）。适配只需两处改动：
...
配置（服务设置、连接 URL）以 JSON 文件形式保存到 `app.getPath('userData')`。EWebsocketMan 有 300ms 防抖自动保存；EHttpServerTool 使用 `PathConfigManager` 类管理。
...
- 修改共享工具配置（vite config、electron-builder、dev 启动脚本）时，**EHttpServerTool 和 EWebsocketTool** 脚手架几乎一致，通常两处需同步更新。EWebsocketMan 和 EPinyinStudy 配置独立。
