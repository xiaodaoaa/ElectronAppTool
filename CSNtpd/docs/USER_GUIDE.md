...
- **顶部标题栏**：应用名称 + 副标题（"局域网时间同步工具 · NTP 客户端 / 服务端"），右侧显示当前权限徽章（"管理员" / "普通用户"）。
- **时间卡片条**：三栏显示 **本地时间** / **UTC 时间** / **当前时间偏差**（每秒刷新；偏差在同步成功前显示为 `-`）。
- **客户端 / 服务端卡片**（左右并排）：
  - **NTP Client 区域**：状态彩点 + 文本（停止/运行中/同步中/成功/失败/告警）、上游服务器、最近同步时间、往返延迟。
    - `立即同步`：立即向上游服务器同步一次。
    - `启动自动同步` / `停止自动同步`：控制定时同步。
    - `修改系统时间`：按最近一次成功同步结果的偏差调整本机时间（需管理员，红色危险按钮）。
  - **NTP Server 区域**：状态彩点 + 文本（停止/启动中/监听中/错误）、监听地址:端口、四项统计数字（总请求 / 有效 / 非法 / 拒绝）、最近客户端。
    - `启动服务端` / `停止服务端`：控制 UDP 监听。
- **日志面板**：表格显示运行日志（时间 / 级别 / 模块 / 消息），最多保留 2000 条（超出自动从头删除旧条目）。
  - `打开配置`：打开图形化设置窗口。
  - `清除日志`：清空当前界面日志显示。

## 4. 系统托盘

关闭主窗口时**最小化到系统托盘**（不会退出程序）。托盘图标右键菜单提供（顺序）：

- `显示主界面`：重新打开主窗口（双击托盘图标亦可）。
- `立即同步`：手动同步一次。
- `启动服务端` / `停止服务端`：快捷启停 NTP 服务端（停止前会弹出确认）。
- `设置`：打开图形化设置界面。
- `退出`：真正结束程序。

> 只有从托盘选择"退出"才会彻底退出程序；关闭主窗口只是最小化到托盘。同步失败时托盘会弹出气泡通知。

## 5. 图形化设置界面

通过主界面 `打开配置` 按钮或托盘菜单 `设置` 打开。设置窗口采用 **Windows 11 风格**（左侧导航 + 右侧卡片），分为 **客户端 / 服务端 / 日志** 三个页面。修改后点 `保存` 即生效并应用，`恢复默认` 还原配置，`取消` 放弃修改。
